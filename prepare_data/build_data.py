...
class MedicalGraph:
    def __init__(self):
        ...
        # 存放清洗后的数据
        self.newdb = self.conn['medical_modify']
    ...
    def collect_medical(self):
        ...
        for item in self.col.find():
            ...
            try:
                ...
                self.newdb['medical_modify'].insert(data_modify)
                count += 1
                print(count)
    ...
    def get_inspect(self, url):
        res = self.newdb['jc'].find_one({'url': url})
        if not res:
            return ''
        else:
            return res['name']
